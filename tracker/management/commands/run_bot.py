...
async def fetch_usd_to_khr_rate(force_refresh=False):
    ...
    CACHE_TTL = 600
    now = time.time()
    if not force_refresh and _EXCHANGE_RATE_CACHE['usd_to_khr'] and (now - _EXCHANGE_RATE_CACHE['timestamp'] < CACHE_TTL):
...
def _all_buttons_extra_rows(lang: str):
    ...
    rows = [
        [
            _make_callback_button(_t(lang, f"{_icon('add')} បន្ថែមចំណាយ", f"{_icon('add')} Add Expense"), "quick_add_expense"),
            _make_callback_button(_t(lang, f"{_icon('add')} បន្ថែមចំណូល", f"{_icon('add')} Add Income"), "quick_add_income"),
        ],
        [
            _make_callback_button(_t(lang, f"{_icon('balance')} សមតុល្យ", f"{_icon('balance')} Balance"), "quick_balance"),
            _make_callback_button(_t(lang, f"{_icon('today')} ថ្ងៃនេះ", f"{_icon('today')} Today"), "quick_today"),
        ],
        [
            _make_callback_button(_t(lang, f"{_icon('month')} ខែនេះ", f"{_icon('month')} This Month"), "quick_month"),
            _make_callback_button(_t(lang, f"{_icon('fx')} KHR", f"{_icon('fx')} USD"), "quick_fx"),
        ],
        [
            _make_callback_button(_t(lang, f"{_icon('recent')} បញ្ជីថ្មីៗ", f"{_icon('recent')} Recent"), "quick_list"),
            _make_callback_button(_t(lang, f"{_icon('summary')} សរុប", f"{_icon('summary')} Summary"), "quick_summary"),
        ],
        [
            _make_callback_button(_t(lang, f"{_icon('help')} ជំនួយ", f"{_icon('help')} Help"), "quick_help"),
        ],
        [
...
def _detect_user_lang(update_like=None, context: ContextTypes.DEFAULT_TYPE | None = None, text_hint: str | None = None) -> str:
    if _is_khmer_text(text_hint):
        lang = LANG_KH
    elif text_hint:
        lang = LANG_EN
    elif context and getattr(context, 'user_data', None) and context.user_data.get('lang') in (LANG_KH, LANG_EN):
...
def _base_menu_rows(lang: str, include_show_all: bool = True, include_lang: bool = True):
    ...
    if include_show_all:
        rows.append([
            _make_callback_button(_t(lang, "បង្ហាញប៊ូតុងទាំងអស់", "Show All Buttons"), "show_all_buttons")
        ])
    if include_lang:
        if lang == LANG_KH:
            rows.append([
...
def _build_start_keyboard(lang: str, extra_rows=None) -> InlineKeyboardMarkup | None:
    ...
    if extra_rows:
        rows = _base_menu_rows(lang, include_show_all=False, include_lang=False) + [r for r in extra_rows if r]
    else:
...
async def send_with_menu(bot, chat_id: int, text: str, lang: str = LANG_EN, **kwargs):
    safe_text = _sanitize_text_icons(text, keep_warning=True)
    return await bot.send_message(chat_id=chat_id, text=safe_text, **_reply_options(lang=lang, **kwargs))
...
async def send_help_message(message, user_name: str, user_id: int, lang: str, currency_mode: str = 'USD'):
...
async def send_recent_transactions(message, user_id: int, lang: str, currency_mode: str = 'USD'):
    ...
    if not transactions:
        await reply_with_menu(
            message,
            _t(lang, f"{_icon('recent')} មិនទាន់មានប្រតិបត្តិការនៅឡើយទេ។", f"{_icon('recent')} No transactions recorded yet."),
            lang=lang,
            extra_rows=_report_extra_rows(lang, currency_mode=currency_mode),
        )
        return

    lines = [_t(lang, f"{_icon('recent')} *ប្រតិបត្តិការថ្មីៗ (10 ចុងក្រោយ)*", f"{_icon('recent')} *Recent Transactions (last 10)*")]
    for tx in transactions:
        icon = _icon('tx_type', tx_type=tx.transaction_type)
        cur = getattr(tx, 'currency', 'USD') or 'USD'
        sym = '$' if cur == 'USD' else '៛'
        note = tx.note or _t(lang, "មិនមាន", "N/A")
        lines.append(f"{icon} {tx.transaction_date}: {sym}{tx.amount:,.2f} ({cur}) - {tx.category_name or tx.category} - {note}")
...
async def send_quick_entry_help(message, entry_type: str, lang: str):
    ...
    await reply_with_menu(message, text, lang=lang, parse_mode=ParseMode.HTML, extra_rows=_entry_extra_rows(lang))
...
async def handle_quick_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "quick_help":
        ...
        help_support_rows = [
            [
                _make_callback_button(_t(lang, "📞 ទាក់ទង Admin", "📞 Contact Admin"), "help_contact_admin"),
                _make_callback_button(_t(lang, "📖 របៀបប្រើប្រាស់", "📖 User Guide"), "help_user_guide"),
            ],
            [
    ...
    if action == "quick_back_main":
        ...
        if prev:
            ...
            if menu == 'main':
                ...
                await send_help_message(query.message, user_name, user_id, lang, currency_mode=currency_mode)
    ...
    await reply_with_menu(
        query.message,
        _t(lang, f"{_icon('info')} ប៊ូតុងនេះមិនទាន់អាចប្រើបានទេ។ សូមព្យាយាមម្ដងទៀត។", f"{_icon('info')} This action is not available right now. Please try again."),
        lang=lang,
        extra_rows=[[ 
            _make_callback_button(_t(lang, "⬅️ ត្រឡប់ក្រោយ", "⬅️ Back"), "quick_back_main")
        ]],
...
async def handle_reply_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang = _detect_user_lang(update, context, update.message.text if update and update.message else None)
    ...
    if not id_match:
        await reply_with_menu(
            update.message,
            _t(lang, f"{_icon('error')} រកមិនឃើញលេខសម្គាល់ប្រតិបត្តិការនៅក្នុងសារដើមទេ។", f"{_icon('error')} Cannot find transaction ID in the original message."),
            lang=lang,
        )
        return
    ...
    if not tx:
        await reply_with_menu(
            update.message,
            _t(lang, f"{_icon('error')} រកមិនឃើញប្រតិបត្តិការ #{tx_id} ឬអ្នកគ្មានសិទ្ធិ។", f"{_icon('error')} Transaction #{tx_id} not found or unauthorized."),
            lang=lang,
        )
        return

    await reply_with_menu(update.message, _t(lang, f"{_icon('thinking')} កំពុងវិភាគសំណើរបស់អ្នក...", f"{_icon('thinking')} Analyzing your request..."), lang=lang)
    ...
    try:
        ...
        if action == 'delete':
            ...
            await reply_with_menu(update.message, response, lang=lang, parse_mode=ParseMode.MARKDOWN, extra_rows=_entry_extra_rows(lang))
            ...
            if not any(v is not None for v in changes.values()):
                await reply_with_menu(
                    update.message,
                    _t(lang, f"{_icon('info')} មិនឃើញការកែប្រែថ្មីទេ។ សូមបញ្ជាក់អ្វីដែលចង់កែ។", f"{_icon('info')} No changes detected. Please specify what to edit."),
                    lang=lang,
            ...
            def apply_edits():
                ...
                if changes.get('amount') is not None:
                    ...
                    if cur == 'USD':
                        ...
                        tx.amount_usd = new_amt / rate if rate else new_amt / rate  # Always use the latest rate, fallback handled in fetch_usd_to_khr_rate
                    updated_fields.append(_t(lang, f"{_icon('balance')} ចំនួន: ${old:.2f} → ${float(new_amt):.2f}", f"{_icon('balance')} Amount: ${old:.2f} → ${float(new_amt):.2f}"))
                ...
                if changes.get('category') is not None:
                    ...
                    try:
                        ...
                        cat_obj, _ = Category.objects.get_or_create(name=new_cat, defaults={'icon': _icon('balance')})
                    tx.category = cat_obj
                    tx.category_name = cat_obj.name
                    updated_fields.append(_t(lang, f"{_icon('category')} ប្រភេទ: {old_cat} → {cat_obj.name}", f"{_icon('category')} Category: {old_cat} → {cat_obj.name}"))
                ...
                if changes.get('date') is not None:
                    ...
                    updated_fields.append(_t(lang, f"{_icon('today')} កាលបរិច្ឆេទ: {old_date} → {changes['date']}", f"{_icon('today')} Date: {old_date} → {changes['date']}"))

                if changes.get('note') is not None:
                    old_note = tx.note or '(empty)'
                    tx.note = changes['note']
                    updated_fields.append(_t(lang, f"{_icon('note')} កំណត់សម្គាល់: {old_note} → {changes['note']}", f"{_icon('note')} Note: {old_note} → {changes['note']}"))
...
async def handle_summary(update: Update, user_id, data, lang: str, currency_mode: str = 'USD'):
    ...
    def fetch_summary():
        ...
        overall_income = base_qs.filter(transaction_type='income').aggregate(t=Sum('amount_usd'))['t'] or Decimal('0')
        overall_expense = base_qs.filter(transaction_type='expense').aggregate(t=Sum('amount_usd'))['t'] or Decimal('0')
    ...
    title = _t(lang, f"{_icon('summary')} របាយការណ៍{s['period_kh']}", f"{_icon('summary')} Financial Summary")
...
async def handle_balance(update: Update, user_id, lang: str, currency_mode: str = 'USD'):
    ...
    if b['top_cats']:
        for i, c in enumerate(b['top_cats'], 1):
            total = c['total'] or Decimal('0')
            cat_name = html.escape(str(c['category_name']))
            cat_lines.append(f"{i}. <b>{cat_name}</b> — <code>${total:,.2f}</code> | <code>៛{total * KHR:,.0f}</code>")
    ...
    try:
        ...
        if not data.get('is_transaction', True):
            ...
            if data.get('is_balance'):
                ...
                return
            response = data.get('message', _t(lang, 'នេះមិនមែនជាប្រតិបត្តិការហិរញ្ញវត្ថុទេ។', 'This does not appear to be a financial transaction.'))
            await reply_with_menu(
                update.message,
        ...
        category, _ = await asyncio.to_thread(Category.objects.get_or_create, name=category_name, defaults={'icon': _icon('balance')})
        ...
        await reply_with_menu(update.message, response, lang=lang, parse_mode=ParseMode.HTML, extra_rows=_entry_extra_rows(lang))
        ...
        if tx_type == 'expense':
            ...
            if net < 0:
                ...
                await send_with_menu(context.bot, user_id, alert, lang=lang, parse_mode=ParseMode.MARKDOWN, extra_rows=_entry_extra_rows(lang))
...
async def cmd_total(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await reply_with_menu(update.message, response, lang=lang, parse_mode=ParseMode.MARKDOWN, extra_rows=_report_extra_rows(lang, currency_mode=currency_mode))
...
async def cmd_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await reply_with_menu(update.message, _t(lang, f"{_icon('error')} Transaction ID ត្រូវតែជាលេខ", f"{_icon('error')} Transaction ID must be a number"), lang=lang)
        return
...
async def cmd_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await reply_with_menu(update.message, _t(lang, f"{_icon('error')} ទម្រង់បញ្ជាមិនត្រឹមត្រូវ", f"{_icon('error')} Invalid format"), lang=lang)
        return
